...
MAUTIC_WEBHOOK_SECRET = os.getenv("MAUTIC_WEBHOOK_SECRET", "")
...
COGNITO_SKIP_LOCAL_PASSWORD_PROVIDERS = {
    p.strip().lower()
    for p in os.getenv("COGNITO_SKIP_LOCAL_PASSWORD_PROVIDERS", "").split(",")
    if p.strip()
}
...
BREAKOUT_RETURN_CLIENT_JITTER_MS = int(os.getenv("BREAKOUT_RETURN_CLIENT_JITTER_MS", "15000"))

SPEED_NETWORKING_STOP_RETURN_JITTER_MS = int(os.getenv("SPEED_NETWORKING_STOP_RETURN_JITTER_MS", "15000"))
...
if MAUTIC_SYNC_ENABLED:
    CELERY_BEAT_SCHEDULE.update({
        ...
        "dispatch-due-newsletter-campaign-send-events": {
            "task": "newsletter.dispatch_due_campaign_send_events",
            "schedule": timedelta(minutes=1),
            "args": (100,),
        },
...
DEFAULT_LEGAL_ENTITY_CODE = os.getenv("DEFAULT_LEGAL_ENTITY_CODE", "CH")
