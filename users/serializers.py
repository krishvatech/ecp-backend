...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ["id", "username", "email", "profile", "first_name", "last_name","is_active", "is_superuser", "is_staff", "date_joined","educations", "experiences", "posts_count", "contacts_count", "provider"]
        read_only_fields = ["id", "is_active","is_superuser", "is_staff",  "date_joined","first_name", "last_name", "provider"]
    ...
    def get_provider(self, obj):
        """Identity provider backing this account: "cognito", "google", etc.

        Lets the frontend decide whether a password can be changed in place
        (native Cognito users) or must be set through the reset flow
        (federated users, whose Cognito password was generated by the backend
        and is unknown to them).

        Prefers the current request's Cognito claims so the answer reflects how
        the user actually signed in, and falls back to the stored
        CognitoIdentity. Defaults to "cognito" so unknown accounts keep the
        existing change-password behaviour.
        """
        request = self.context.get("request")
        claims = getattr(request, "cognito_claims", None) if request else None
        if isinstance(claims, dict) and claims:
            try:
                from .cognito_auth import _provider_from_claims
                ...
                if provider:
                    return provider
            except Exception:  # pragma: no cover - never break profile reads
                pass

        try:
            identity = obj.cognito_identities.order_by("-id").first()
            if identity and identity.provider:
...
class ProfileVisitorSerializer(serializers.Serializer):
    ...
    first_name = serializers.CharField(source="viewer.first_name", read_only=True, allow_null=True)
    last_name = serializers.CharField(source="viewer.last_name", read_only=True, allow_null=True)
    ...
    kyc_status = serializers.CharField(source="viewer.profile.kyc_status", read_only=True, allow_null=True)
