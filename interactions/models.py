...
class Question(models.Model):
    ...
    adopted_suggestion = models.ForeignKey(
...
# -----------------------------------------------------------------
# A3: Public AI Question Suggestions
# -----------------------------------------------------------------

class QnAAIPublicSuggestion(models.Model):
    """
    Candidate questions generated by AI (or manually by host) that can
    be published for all participants to see and adopt.
    """
    event = models.ForeignKey(
        "events.Event",
        on_delete=models.CASCADE,
        related_name="qna_public_suggestions"
    )
    session = models.ForeignKey(
        "events.EventSession",
        on_delete=models.CASCADE,
    ...
    SOURCE_AI = "ai"
    SOURCE_MANUAL = "manual"
    SOURCE_CHOICES = [(SOURCE_AI, "AI"), (SOURCE_MANUAL, "Manual")]
    source_type = models.CharField(max_length=10, choices=SOURCE_CHOICES, default=SOURCE_AI)
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="created_qna_suggestions"
    )
    reviewed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="reviewed_qna_suggestions"
    )
    ...
    class Meta:
        ordering = ["display_order", "-created_at"]
        verbose_name = "AI Public Suggestion"
        verbose_name_plural = "AI Public Suggestions"

    def __str__(self) -> str:
        return f"[{self.status}] {self.question_text[:50]}"


class QnAAIPublicSuggestionAdoption(models.Model):
    """
    Analytical logging for when a participant 'adopts' a suggestion.
    Helps track performance of AI suggestions.
    """
    ...
    event = models.ForeignKey(
        "events.Event",
        on_delete=models.CASCADE,
        related_name="qna_suggestion_adoptions"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
    ...
    submitted_question = models.ForeignKey(
        "Question",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    class Meta:
        verbose_name = "AI Suggestion Adoption"
        verbose_name_plural = "AI Suggestion Adoptions"
        constraints = [
            models.CheckConstraint(
                name="adoption_has_user_or_guest",
                check=models.Q(user__isnull=False) | models.Q(guest__isnull=False),
            ),
        ]

    def __str__(self) -> str:
        who = f"U{self.user_id}" if self.user_id else f"G{self.guest_id}"
        return f"{who} adopted S{self.suggestion_id}"
